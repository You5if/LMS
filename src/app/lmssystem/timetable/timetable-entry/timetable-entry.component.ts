...
export class TimetableEntryComponent implements OnInit {
  ...
  constructor(
	  private dapiService: TimetableEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _auth: AuthService,
      private _globals: AppGlobals,
      private _select: SelectService,
      private dialogRef: MatDialogRef<TimetableEntryComponent>,
      @Inject(MAT_DIALOG_DATA) public pModel: Send
  ) { }

  ngOnInit() {

    console.log('Here:',this.pModel);
  ...
  onSubmit() {
    ...
    for(let i=0;i<=this.data.length;i++){
     ...
     console.log("timetablearray", JSON.stringify(this.last));
