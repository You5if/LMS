...
import { QuestionBankEntryService } from './questionbank-entry.service';
import { QuBanMCEntryComponent } from '../../qubanmc/qubanmc-entry/qubanmc-entry.component';
import { QuBanMCService } from '../../qubanmc/qubanmc.service';
@Component({
  selector: 'app-questionbank-entry',
  templateUrl: './questionbank-entry.component.html',
  styleUrls: ['./questionbank-entry.component.scss'],
 
})

export class QuestionBankEntryComponent implements OnInit {
  ...
  constructor(
	  private dapiService: QuestionBankEntryService,
      private _ui: UIService,
      private _msg: MessageBoxService,
      private _auth: AuthService,
      private _globals: AppGlobals,
      private _select: SelectService,
      private dialogRef: MatDialogRef<QuestionBankEntryComponent>,
  ...
  deleteFun4(id: number) {
    ...
    onSubmit() {
      ...
      for (let i = 0; i < this.lastDark.child2.length; i++) {
        this.lastDark.child2[i].records.sort(function(a, b) { 
          return a.applicationOrder - b.applicationOrder  ||  a.label.localeCompare(b.label);
        });
      }
      for (let i = 0; i < this.lastDark.child3.length; i++) {
        this.lastDark.child3[i].records.sort(function(a, b) { 
          return a.applicationOrder - b.applicationOrder  ||  a.label.localeCompare(b.label);
        });
      }
      for (let i = 0; i < this.lastDark.child4.length; i++) {
        this.lastDark.child4[i].records.sort(function(a, b) { 
          return a.applicationOrder - b.applicationOrder  ||  a.label.localeCompare(b.label);
        });
      }
