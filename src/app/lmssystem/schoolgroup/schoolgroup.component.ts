...
import { SchoolgroupEntryComponent } from './schoolgroup-entry/schoolgroup-entry.component';
import { SchoolGroupModel } from './schoolgroup.model';
import { RightModel } from 'src/app/components/security/auth/rights.model';
import { RouterModule, Routes } from '@angular/router';
import { PageSortComponent } from 'src/app/components/common/pageevents/page-sort/page-sort.component';
import { SchoolGroupService } from './schoolgroup.service';
...
@Component({
    ...
    styleUrls: ['./schoolgroup.component.scss']
  })

export class SchoolGroupComponent implements OnInit {

    displayedColumns: string[] =
        ['select','scName', 'scClassName', 'scGroupName'];
    ...
    clickedRows = new Set<SchoolGroupModel>();
    selection = new SelectionModel<SchoolGroupModel>(true, []);
  ...
  refreshMe() {
    ...
    this._cf.getPageData('SchoolGroup', this.pScreenId, this._auth.getUserId(), this.pTableId,
  ...
  onId(id: number, row:SchoolGroupModel) {
  ...
  onView = (id: number) => {
    ...
    this.schoolgroupservice.getSchoolGroupEntry(id).subscribe((result: SchoolGroupModel) => {
      this._ui.loadingStateChanged.next(false);
      result.entryMode = 'V';
  ...
  openEntry = function (result: SchoolGroupModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(SchoolgroupEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
