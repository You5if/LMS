...
export class AccountConfigurationComponent implements OnInit {
  ...
  indexes: AccountConfigurationModel;
    ...
    try {
      this._cf.getPageDataOnPaginatorOperation(event, this.pTableName, this.pScreenId, this._auth.getUserId(),
        this.pTableId, this.totalRecords).subscribe(
          (result: AccountConfigurationModel) => {
  ...
  openEntry = function (result: AccountConfigurationModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(AccountConfigurationEntryComponent, {
        disableClose: true,
        maxWidth: '100vw',
      maxHeight: '100vh',
      height: '100%',
