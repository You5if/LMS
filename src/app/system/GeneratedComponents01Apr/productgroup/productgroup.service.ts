...
export class ProductGroupService {
   ...
   getProductGroupEntry(id: number): Observable<ProductGroupModel> {
      return this.httpClient.get<ProductGroupModel>(this._globals.baseAPIUrl + 'ProductGroup/' + id).pipe(
   ...
   getProductGroupSubmit(data: ProductGroupModel) {
      data.auditColumns = {
      'userId': 1,
      'hostname': 'test',
      'ipaddress': 'test',
      ...
      switch (data.entryMode) {
          ...
          case 'A': {
          return this.http.post(this._globals.baseAPIUrl + 'ProductGroup/create', data, this._cf.requestOptions()).pipe(
          ...
          case 'E': {
          return this.http.post(this._globals.baseAPIUrl + 'ProductGroup/edit', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
          ...
          case 'D': {
          return this.http.post(this._globals.baseAPIUrl + 'ProductGroup/delete', data, this._cf.requestOptions()).pipe(
          map((response: Response) => {
          return response.json();
          }), catchError(this._cf.handleError));
          }
