...
export class CompanyBankBranchComponent implements OnInit {
  ...
  clickedRows = new Set<CompanyBankBranchModel>();
    ...
    menuId: number;
    indexes: CompanyBankBranchModel
  ...
  openEntry = function (result: CompanyBankBranchModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(CompanyBankBranchEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
