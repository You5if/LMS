...
export class ProductPricingComponent implements OnInit {
  ...
  openEntry = function (result: ProductPricingModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(ProductPricingEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
