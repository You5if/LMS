...
import { RegistrationInvoiceEntryComponent } from './invoice-entry/invoice-entry.component';
...
export class RegistrationInvoiceComponent implements OnInit {
  ...
  refreshMe() {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      this.direction = "ltr"
      this.header = "Registration invoice"
  ...
  openEntry = function (result: InvoiceModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(RegistrationInvoiceEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
