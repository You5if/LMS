...
openState = function (result: any) {
  this.dialogRef = this.dialog.open(CheckforstateComp2Component, {
    disableClose: true,
...
onMyFilter = function () {
  ...
  openEntry = function (result: PaymentToCompanyModel) {
    if (result === undefined) {
      this.dialogRef = this.dialog.open(PaymentfromEntryComponent, {
        disableClose: true,
        data: {}
      });
    } else {
